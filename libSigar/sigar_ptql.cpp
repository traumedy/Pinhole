...
static int ptql_branch_list_create(ptql_branch_list_t *branches)
{
    ...
    branches->data = reinterpret_cast<ptql_branch_t*>(malloc(sizeof(*(branches->data)) *
                            branches->size));
...
{
    branches->data =
        reinterpret_cast<ptql_branch_t*>(realloc(branches->data,
                                     ...
                                     LPQUERY_SERVICE_CONFIGA config)
{
    ...
    DWORD bytes;
    SC_HANDLE handle =
        OpenServiceA(scm_handle, name, SERVICE_QUERY_CONFIG);

    if (!handle) {
        return GetLastError();
    }

    if (QueryServiceConfigA(handle, config, QUERY_SC_SIZE, &bytes)) {
        status = SIGAR_OK;
    }
    else {
        status = GetLastError();
