...
int sigar_cpu_info_list_create(sigar_cpu_info_list_t *cpu_infos)
{
    ...
    cpu_infos->data = (sigar_cpu_info_t*)malloc(sizeof(*(cpu_infos->data)) *
                             cpu_infos->size);
    return SIGAR_OK;
}
...
{
    cpu_infos->data = (sigar_cpu_info_t*)realloc(cpu_infos->data,
...
int sigar_cpu_list_create(sigar_cpu_list_t *cpulist)
{
    ...
    cpulist->data = (sigar_cpu_t*)malloc(sizeof(*(cpulist->data)) *
                           cpulist->size);
    return SIGAR_OK;
}
...
{
    cpulist->data = (sigar_cpu_t*)realloc(cpulist->data,
                            sizeof(*(cpulist->data)) *
                            (cpulist->size + SIGAR_CPU_INFO_MAX));
    cpulist->size += SIGAR_CPU_INFO_MAX;
                                         ...
                                         sigar_net_connection_t *conn)
{
    ...
    if (entry->value) {
        if (conn->local_address.family == sigar_net_address_t::SIGAR_AF_INET6) {
            return; /* prefer ipv4 */
        }
    }
    else {
