...
#define MyRegQueryValue() \
    (USING_WIDE() ? \
        RegQueryValueExW(sigar->handle, \
                         wcounter_key, NULL, (LPDWORD)&type, \
                         (LPBYTE)performanceBuffer->buffer, \
                         (LPDWORD)&bytes) : \
        RegQueryValueExA(sigar->handle, \
                         counter_key, NULL, (LPDWORD)&type, \
                         (LPBYTE)performanceBuffer->buffer, \
                         (LPDWORD)&bytes))
...
{
    if (!buffer->buffer) {
		buffer->buffer = reinterpret_cast<char*>(malloc(BUFFER_SIZE));
...
static DWORD buffer_grow(buffer_t *buffer)
{
	...
	buffer->buffer =
		reinterpret_cast<char*>(realloc(buffer->buffer, buffer->size));
                                              ...
                                              DWORD inst, DWORD *err)
{
    ...
    *err = get_performance_buffer_by_counter_key(sigar, counter_key, &performanceBuffer);
	...
	performanceBuffer->buffer = (char*)block;
...
static int get_mem_counters(sigar_t *sigar, sigar_swap_t *swap, sigar_mem_t *mem)
{
    int status;
    PERF_OBJECT_TYPE *object =
        get_perf_object_inst(sigar, PERF_TITLE_MEM_KEY, 0, reinterpret_cast<DWORD*>(&status));
                             ...
                             int all)
{
    ...
    if (!(success = (module->handle ? TRUE : FALSE))) {
        rc = GetLastError();
        /* dont try again */
        module->handle = (HINSTANCE)INVALID_HANDLE_VALUE;
    }
...
static int netif_name_short(void)
{
    ...
    DWORD retval =
        GetEnvironmentVariableA("SIGAR_NETIF_NAME_SHORT", value, sizeof(value));
...
int sigar_os_open(sigar_t **sigar_ptr)
{
    ...
    *sigar_ptr = sigar = (sigar_t *)malloc(sizeof(*sigar));
	...
	sigar_enable_privilege("SeDebugPrivilege");
                                    ...
                                    sigar_uptime_t *uptime)
{
    int status;
    PERF_OBJECT_TYPE *object =
        get_perf_object_inst(sigar, PERF_TITLE_SYS_KEY, 0, (DWORD*)&status);
    PERF_COUNTER_DEFINITION *counter;
    BYTE *data;
    DWORD i;
    ...
    if (sigar_EnumProcesses) {
		...
		do {
			if (size == 0) {
				...
				size = buffer_grow((buffer_t*)(sigar));
			}
                         ...
                         sigar_proc_cred_name_t *proccredname)
{
    ...
    if (success) {
        ...
        success = LookupAccountSidA(NULL, user->User.Sid,
    ...
    if (success) {
        ...
        success = LookupAccountSidA(NULL, group->PrimaryGroup,
                                ...
                                int multi)
{
    while (*ptr) {
        ...
        val = (char*)strchr((const char*)ptr, '=');
                                    ...
                                    sigar_proc_env_t *procenv)
{
    ...
    FreeEnvironmentStringsA((LPCH)env);
                                      ...
                                      sigar_proc_env_t *procenv)
{
    if (pid == sigar->pid) {
        if (procenv->type == sigar_proc_env_t::SIGAR_PROC_ENV_KEY) {
            char value[32767]; /* max size from msdn docs */
            DWORD retval = 
                GetEnvironmentVariableA(procenv->key, value, sizeof(value));
                                              ...
                                              sigar_file_system_list_t *fslist)
{
    ...
    while (*ptr) {
        ...
        char fsname[1024];
        UINT drive_type = GetDriveTypeA(ptr);
        int type;
        ...
        GetVolumeInformationA(ptr, NULL, 0, &serialnum, NULL,
        ...
        fsp->type = (sigar_file_system_type_e)type;
                            ...
                            sigar_file_system_usage_t *fsusage)
{
    ...
    retval = GetDiskFreeSpaceExA(dirname,
                                &avail, &total, &free);
...
static int sigar_cpu_info_get(sigar_t *sigar, sigar_cpu_info_t *info)
{
    ...
    rc = RegEnumKeyA(key, 0, id, sizeof(id));
    if (rc != ERROR_SUCCESS) {
        RegCloseKey(key);
        return rc;
    }
       
    rc = RegOpenKeyA(key, id, &cpu);
    if (rc != ERROR_SUCCESS) {
        RegCloseKey(key);
        return rc;
    }
    ...
    if (RegQueryValueExA(cpu, "VendorIdentifier", NULL, NULL,
                        (LPBYTE)&info->vendor, &size) ||
    ...
    if (RegQueryValueExA(cpu, "ProcessorNameString", NULL, NULL,
                        (LPBYTE)&info->model, &size))
    {
        size = sizeof(info->model);
        if (RegQueryValueExA(cpu, "Identifier", NULL, NULL,
                            (LPBYTE)&info->model, &size))
        {
    ...
    if (RegQueryValueExA(cpu, "~MHz", NULL, NULL,
                        (LPBYTE)&info->mhz, &size))
    {
        info->mhz = -1;
    }
                                   ...
                                   PIP_ADAPTER_INFO *adapter)
{
    ...
    *adapter = (PIP_ADAPTER_INFO)sigar->ifconf_buf;
    ...
    if (rc == ERROR_BUFFER_OVERFLOW) {
        ...
        sigar->ifconf_len = size;
        sigar->ifconf_buf = (char*)realloc(sigar->ifconf_buf,
                                    sigar->ifconf_len);
                                        ...
                                        ULONG *size)
{
    ...
    if (rc == ERROR_BUFFER_OVERFLOW) {
        ...
        *addrs = (PIP_ADAPTER_ADDRESSES)realloc(*addrs, *size);
                                            ...
                                            sigar_net_route_list_t *routelist)
{
    ...
    buffer = (PMIB_IPFORWARDTABLE)malloc(bufsize);
    rc = sigar_GetIpForwardTable(buffer, &bufsize, FALSE);
    if (rc != NO_ERROR) {
        free(buffer);
        return GetLastError();
                                                ...
                                                sigar_net_interface_config_t *ifconfig)
{
    ...
    status = sigar_get_adapters_addresses(sigar, AF_UNSPEC, flags, &aa, (ULONG*)&sigar->ifconf_len);
                                      ...
                                      sigar_arp_list_t *arplist)
{
    ...
    ipnet = (PMIB_IPNETTABLE)malloc(size);
                          ...
                          sigar_who_t *who)
{
    ...
    if (RegOpenKeyA(users, key_name, &key) != ERROR_SUCCESS) {
        return ENOENT;
    }
    ...
    size = sizeof(value);
    status = RegQueryValueExA(key, "CLIENTNAME",
    ...
    size = sizeof(value);
    status = RegQueryValueExA(key, "SESSIONNAME",
                             NULL, &type, (LPBYTE)value, &size);
    if (status == ERROR_SUCCESS) {
                              ...
                              sigar_who_list_t *wholist)
{
    ...
    while (1) {
        ...
        status = RegEnumKeyExA(users, index, subkey, &subkey_len,
                              NULL, NULL, NULL, NULL);
        ...
        if (LookupAccountSidA(NULL, /* server */
                             sid, 
...
int sigar_service_pid_get(sigar_t *sigar, char *name, sigar_pid_t *pid)
{
    ...
    if (!(svc = OpenServiceA(mgr, name, SERVICE_ALL_ACCESS))) {
...
int sigar_services_status_get(sigar_services_status_t *ss, DWORD state)
{
    ...
    retval = EnumServicesStatusA(ss->handle,
                                SERVICE_WIN32, state,
                                ss->services, ss->size,
    ...
    if (retval == FALSE) {
        ...
        ss->services = (ENUM_SERVICE_STATUSA*)realloc(ss->services, bytes);
        ss->size = bytes;
                           ...
                           sigar_proc_env_t *infocb)
{
    ...
    if (!(len = GetFileVersionInfoSizeA(name, &handle))) {
        return GetLastError();
    }
    ...
    if (GetFileVersionInfoA(name, handle, len, data)) {
        if (VerQueryValueA(data, "\\", (LPVOID*)&info, (PUINT)&len)) {
    ...
    if (infocb && (status == SIGAR_OK)) {
        ...
        if (VerQueryValueA(data, "\\VarFileInfo\\Translation",
                          (LPVOID*)&trans, (PUINT)&len))
        {
            int i;
            char buf[1024];
            void *ptr;
            ...
            for (i=0; string_file_info_keys[i]; i++) {
                ...
                sprintf(buf, "\\StringFileInfo\\%04x%04x\\%s",
                        ...
                        key);
                if (VerQueryValueA(data, buf, &ptr, (PUINT)&len)) {
                    if (len == 0) {
                        continue;
                    }
