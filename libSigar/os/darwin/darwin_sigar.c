...
static int proc_fdinfo_get(sigar_t *sigar, sigar_pid_t pid, int *num)
{
    ...
    if (sigar->ifconf_len == 0) {
        ...
        sigar->ifconf_buf = (char*)malloc(sigar->ifconf_len);
    }
                           ...
                           sigar_proc_list_t *proclist)
{
    ...
    proc = (struct kinfo_proc *)malloc(len);
...
static int sigar_get_pinfo(sigar_t *sigar, sigar_pid_t pid)
{
    ...
    if (sigar->pinfo == NULL) {
        sigar->pinfo = (bsd_pinfo_t *)malloc(len);
    }
                      ...
                      sigar_proc_fd_t *procfd)
{
    ...
    ofiles = (struct file **)malloc(size);
                               ...
                               sigar_file_system_list_t *fslist)
{
    ...
    len = sizeof(*fs) * num;
    fs = (struct sigar_statfs *)malloc(len);
...
static int sigar_ifmsg_init(sigar_t *sigar)
{
    ...
    if (sigar->ifconf_len < len) {
        sigar->ifconf_buf = (char*)realloc(sigar->ifconf_buf, len);
    ...
    while (ptr < end) {
        ...
        switch (iter->type) {
          case IFMSG_ITER_LIST:
            ...
            name = (char*)malloc(sdl->sdl_nlen+1);
                            ...
                            sigar_nfs_client_v3_t *nfs)
{
    ...
    map_nfs_stats((sigar_nfs_v3_t *)nfs, (rpc_cnt_t *)&stats.rpccnt[0]);
#else
    int status;
    struct nfsstats stats;
                            ...
                            sigar_nfs_server_v3_t *nfs)
{
    ...
    map_nfs_stats((sigar_nfs_v3_t *)nfs, (rpc_cnt_t *)&stats.srvrpccnt[0]);
#else
    int status;
    struct nfsstats stats;
                        ...
                        unsigned long port, sigar_pid_t *pid)
{
    ...
    for (i=0; i<nentries; i++) {
        ...
        if (pinfo[i].KI_FD) {
            ...
            ofiles = (struct file **)malloc(osize); /* XXX reuse */
            if (!ofiles) {
                return errno;
            }
