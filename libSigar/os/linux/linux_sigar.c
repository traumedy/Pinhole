...
static const char *get_hw_type(int type)
{
    switch (type) {
    case ARPHRD_AX25:
        return "ax25";
                       ...
                       sigar_arp_list_t *arplist)
{
    ...
    while (fgets(buffer, sizeof(buffer), fp)) {
        ...
        if (status > 0) {
            arp->address.family = sigar_net_address_t::SIGAR_AF_INET;
        }
        else if ((status = inet_pton(AF_INET6, net_addr, &arp->address.addr)) > 0) {
            arp->address.family = sigar_net_address_t::SIGAR_AF_INET6;
        }
        else {
            sigar_log_printf(sigar, SIGAR_LOG_WARN,
        ...
        arp->hwaddr.family = sigar_net_address_t::SIGAR_AF_LINK;
