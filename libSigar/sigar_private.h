...
// JEBNOTE - disabling inline, breaking Linux builds
#elif defined(__GNUC__xx)
...
#define SIGAR_SSTRCPY(dest, src) \
    SIGAR_STRNCPY(dest, (const char*)src, sizeof(dest))
...
#define sigar_net_address_set(a, val) \
    (a).addr.in = val; \
    (a).family = sigar_net_address_t::SIGAR_AF_INET
...
#define sigar_hwaddr_set_null(ifconfig) \
    ...
    ifconfig->hwaddr.family = sigar_net_address_t::SIGAR_AF_LINK
...
#define sigar_net_interface_ipv6_config_init(ifconfig) \
    ifconfig->address6.family = sigar_net_address_t::SIGAR_AF_INET6; \
