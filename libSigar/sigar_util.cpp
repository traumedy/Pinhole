                               ...
                               const char *dirname)
{
    ...
    if (is_dev) {
        sigar_iodev_t *iodev;
        entry->value = malloc(sizeof(*iodev));
		iodev = (sigar_iodev_t *)entry->value;
        SIGAR_ZERO(iodev);
    ...
    for (i=0; i<fslist.number; i++) {
        ...
        if (fsp->type == SIGAR_FSTYPE_LOCAL_DISK) {
            ...
            if (SIGAR_NAME_IS_DEV(fsp->dev_name)) {
                sigar_iodev_t *iodev;
                ent->value = malloc(sizeof(*iodev));
				iodev = (sigar_iodev_t *)ent->value;
                SIGAR_ZERO(iodev);
                        ...
                        sigar_cpuid_t *id)
{
    ...
    __cpuid(reinterpret_cast<int*>(info), request); /* as of MSVC 7 */
