...
void UGlobalHotkeys::registerHotkey(const UKeySequence& keySeq, size_t id) {
	...
	HWND hwndWin = (HWND)winId();
	//qDebug() << "Window handle: " << hwndWin;
#if 0
	char classBuff[256];
	if (!GetClassNameA(hwndWin, classBuff, sizeof(classBuff)))
	{
		DWORD err = GetLastError();
		qDebug() << "GetClassName Error " << err;
	}
	else
	{
		char nameBuff[256] = "";
		GetWindowTextA(hwndWin, nameBuff, sizeof(nameBuff));
		qDebug() << classBuff << ":" << nameBuff;
	}
#endif
    if (!RegisterHotKey(hwndWin, (int)id, (UINT)winMod, (UINT)key)) {
...
bool UGlobalHotkeys::winEvent(MSG * message, long * result) {
    Q_UNUSED(result);
	//qDebug() << "Message window handle: " << message->hwnd;
